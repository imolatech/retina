...
public class ViewerPanel extends JPanel implements Runnable {
	...
	/*
	 * build a new histogram of 8-bit depth values, and convert it to image
	 * pixels (as bytes)
	 */
	{
	...
	private void calcHistogram(ShortBuffer depthBuf) {
		...
		/*
		 * convert cummulative depth into 8-bit range (0-255), which will later
		 * become grayscales - darker means further away, although black means
		 * "too close" for a depth value to be calculated).
		 */
		if (numPoints > 0) {
			for (int i = 1; i <= maxDepth; i++)
